...
int fillTile(int n) {
    ...
    dp[1] = 1;	// 너비가 1인 경우는 1x2로 채우는 방법 1가지 밖에 없음 
    dp[2] = 3;	// 너비가 2인 경우는 (1) 1x2를 2개 (2) 2x1를 2개 (3) 2x2를 1개 -> 총 3가지 방법 존재함 
    for (int i = 3; i <= n; i++) {
        dp[i] = dp[i - 1] + 2 * dp[i - 2];
        /*	
			따라서 i 번째  타일 채우는 경우의 수는
			i - 1 번째 경우의 수 +  i -2 번째 경우의 수 x 2 이다
			i - 2 번째 경우의 수에 x2를 하는 이유는
			dp[i-2]에 (1) 2x1 타일 2개로 채우는 경우
					  (2) 2x2 타일 1개로 채우는 경우 
			들을 더해줘야 하기 때문 					
		*/   
